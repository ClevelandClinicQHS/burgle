...
test_that("burgle_rfsrc mock competing-risk prediction validates missing cause or times", {
  ...
  expect_error(predict(cr_fit, newdata = data.frame(x = 1:2), type = "risk", times = c(1, 2)), "Please specificy")
  expect_error(predict(cr_fit, newdata = data.frame(x = 1:2), type = "risk", cause = 1), "Please specificy")
})
